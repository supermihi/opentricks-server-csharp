...
public class AuctionTests
{
  private static IDeclarableContract AllowingContract =>
      Mock.Of<IDeclarableContract>(c => c.IsAllowed(It.IsAny<IPlayerCards>()) == true, MockBehavior.Strict);

  private static IByPlayer<bool> NoCompulsorySolos => Mock.Of<IByPlayer<bool>>(MockBehavior.Loose);
  private static IPlayersCards MockCards => Mock.Of<IPlayersCards>();
  ...
  {
    var auction = new Auction(MockCards, NoCompulsorySolos);
    Asserts.ThrowsErrorCode(ErrorCodes.NotYourTurn, () => auction.DeclareReservation(player, AllowingContract));
  }
  ...
  public void ThrowsIfContractNotAllowed()
  {
    var contract = Mock.Of<IDeclarableContract>(
      c => c.IsAllowed(It.IsAny<IPlayerCards>()) == false,
      MockBehavior.Strict);
    var auction = new Auction(MockCards, NoCompulsorySolos);
    Asserts.ThrowsErrorCode(ErrorCodes.ContractNotAllowed, () => auction.DeclareReservation(auction.Turn!.Value, contract));
  }
  ...
  {
    var state = new InTurns<Declaration>(Player.One, Declaration.Ok, Declaration.Ok, Declaration.Ok, Declaration.Ok);
  ...
  {
    var state = new InTurns<Declaration>(Player.One, Enumerable.Repeat(Declaration.Ok, numPlayersDeclared));
    var auction = new Auction(MockCards, NoCompulsorySolos, state);
  ...
  {
    var state = new InTurns<Declaration>(Player.One,
      Declaration.Ok,
